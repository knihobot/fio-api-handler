...
export default class EnhacedFioApi extends FioApi {
  ...
  public processTransactionResponse(data: any): TransactionsResponse {
    ...
    return {
      ...
      transactions: accountStatement.transactionList.transaction.map(
        (transaction: any) => {
  ...
  public async getTransactions(
    ...
    const { data } = await FioApi.prototype.getTransactions.call(
      this,
      start,
      end
    );
  ...
  public async getTransactionOverview(
    year: number,
    overviewNumber: number
  ): Promise<TransactionsResponse> {
    const { data } = await FioApi.prototype.getTransactionOverview.call(
      this,
      year,
      overviewNumber
    );
